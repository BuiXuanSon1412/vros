...
def addNewTripInDroneRoute(
    solution, ChangesPackages, ChangeInTruck, IndexOfNewReceiveCity
):
    ...
    max_point = -1
    for a in range(
        IndexOfNewReceiveCity - 1, 0, -1
    ):  # Tìm min_point_check trên new_solution1[1]
        if solution[0][ChangeInTruck][a][1] != []:
            min_point = solution[0][ChangeInTruck][a][0]
            break
    for a in range(
        IndexOfNewReceiveCity + 1, len(solution[0][ChangeInTruck])
    ):  # Tìm max_point_check trên new_solution1[1]
        if solution[0][ChangeInTruck][a][1] != []:
            max_point = solution[0][ChangeInTruck][a][0]
            break

    start_check_point_in_drone_queue = None
    ...
    assert start_check_point_in_drone_queue is not None, (
        "start_check_point_in_drone_queue must be specified"
    )
        ...
        for a in range(start_check_point_in_drone_queue, len(solution[1])):
            for b in range(len(solution[1][a])):
                if solution[1][a][b][0] == max_point:
                    ...
                    check_max = True
                    break
            if total_time < Function.max_release_date_update(
                solution[1][a]
            ) + Function.cal_time_fly_a_trip(solution[1][a]):
                number_in_the_drone_queue_of_drop_package = a
                check_max = True
                break
            if check_max:
    ...
    solution[1].insert(
        number_in_the_drone_queue_of_drop_package, [[NewReceiveCity, package]]
    )
    index_in_trip = 0
...
# Phục vụ cho nei Neighborghood_change_drone_route, đọc lúc sau
def groupTripInDroneRoute(
    solution, ChangesPackages, ChangeInTruck, IndexOfNewReceiveCity
):
    ReceiveCity = solution[0][ChangeInTruck][IndexOfNewReceiveCity][0]
    count = len(solution[0][ChangeInTruck][IndexOfNewReceiveCity][1]) - len(
        ChangesPackages
    )
    index_drone_trip = -1
    index_in_trip = -1
    ...
    for i in range(len(solution[1])):
        for j in range(len(solution[1][i])):
            if solution[1][i][j][0] == ReceiveCity:
                if (
                    Function.total_demand(solution[1][i])
                    + Function.sum_weight(ChangesPackages)
                    <= Data.drone_capacity
                ):
                    index_drone_trip = i
                    index_in_trip = j
                    for k in range(len(ChangesPackages)):
                        solution[1][i][j][1] = solution[1][i][j][1] + [
                            ChangesPackages[k]
                        ]
    ...
    solution, index_drone_trip, index_in_trip = addNewTripInDroneRoute(
        solution, ChangesPackages, ChangeInTruck, IndexOfNewReceiveCity
    )
    return solution, index_drone_trip, index_in_trip, False


def findIndexOfDropPackage(
    new_solution, solution, city_change, truck_time, i, j, k, l, drop_package
):
    ...
    assert Function.max_release_date(new_solution[0][i][0][1]) is not None, ""
    assert DIFFERENTIAL_RATE_RELEASE_TIME is not None, ""
    assert Data.standard_deviation is not None, ""

    if demand_drop_package == 0:
        new_solution[0][i][0][1] += [drop_package]
        stop = True
    elif (
        Function.max_release_date(new_solution[0][i][0][1])
        * DIFFERENTIAL_RATE_RELEASE_TIME
        + Data.standard_deviation
        > Data.release_date[drop_package]
        and Function.min_release_date(new_solution[0][i][0][1])
        * DIFFERENTIAL_RATE_RELEASE_TIME
        + C_ratio * Data.standard_deviation
        > Data.release_date[drop_package]
    ):
        new_solution[0][i][0][1] += [drop_package]
        stop = True
        ...
        for m in range(len(new_solution[1])):
            for n in range(len(new_solution[1][m])):
                ...
                if (
                    a == max_index
                ):  # nếu tới max_index thì dừng xét và và chuyển sang tạo trip cho drone
                    stop1 = True
                    break
                if a in list_check:
                    # Kiểm tra điều kiện để có thể gộp
                    if (
                        Function.total_demand(new_solution[1][m]) + demand_drop_package
                        <= Data.drone_capacity
                        and Function.max_release_date_update(new_solution[1][m])
                        * DIFFERENTIAL_RATE_RELEASE_TIME
                        + Data.standard_deviation
                        > Function.max_release_date([drop_package])
                        and Function.min_release_date_update(new_solution[1][m])
                        * DIFFERENTIAL_RATE_RELEASE_TIME
                        + B_ratio * Data.standard_deviation
                        > Function.max_release_date([drop_package])
                    ):
                        stop1 = True
                        stop = True
                        ...
                        for c in range(len(new_solution[0][i])):
                            if new_solution[0][i][c][0] == a:
                                ...
                                break
                        new_solution[0][i][number_of_add_city_in_truck][1] += [
                            drop_package
                        ]
    ...
    if not stop:
        ...
        if i == k:  # Trường hợp khi city change vẫn nằm tại truck cũ
            if j < l:  # Trường hợp city change được chuyển ra sau
                ...
                minus = (
                    truck_time[i][j + 1]
                    - truck_time[i][j]
                    - Data.manhattan_move_matrix[solution[0][i][j - 1][0]][
                        solution[0][i][j + 1][0]
                    ]
                    + Data.manhattan_move_matrix[city_change][solution[0][i][j + 1][0]]
                )
                add1 = Data.manhattan_move_matrix[solution[0][i][l][0]][city_change]
                add2 = 0
                if l != len(new_solution[0][i]) - 1:
                    add2 = (
                        add1
                        + Data.manhattan_move_matrix[city_change][
                            solution[0][i][l + 1][0]
                        ]
                    )
                minn = 100000000000
                ...
                for a in range(j - 1, len(new_solution[0][i])):
                    ...
                    if a > where_city_come_after_by_truck_route:
                        if (
                            Data.euclid_flight_matrix[0][point_evaluate] * 2
                            + Data.unloading_time
                        ...
                        if (
                            ...
                            if criterion < minn:
                                index = a
                                minn = criterion
                    if point_evaluate == drop_package:
                        break
        ...
        if index != -1:  # Nếu tìm được index
            ...
            new_solution, index_drone_trip, index_in_trip = addNewTripInDroneRoute(
                new_solution, [drop_package], i, index
            )
        else:
            new_solution[0][i][0][1] += [drop_package]
    return new_solution
            ...
            for j in range(i + ran - 1, middle + 1):
                for k in range(middle + 1, length + 1):
                    for l in range(k + ran - 1, length + 1):
                        ...
                        new_solution[0][aa] = (
                            new_solution[0][aa][:i]
                            + new_solution[0][aa][k : l + 1]
                            + new_solution[0][aa][j + 1 : k]
                            + new_solution[0][aa][i : j + 1]
                            + new_solution[0][aa][l + 1 :]
                        )
                        ...
                        for x in range(len(drop_package)):
                            ...
                            # print(new_solution)
                            new_solution = findLocationForDropPackage(
                                new_solution, aa, drop_package[x]
                            )
...
def findLocationForDropPackage(new_solution, index_truck, drop_package):
    ...
    max_index = (
        -1
    )  # Tìm điểm cuối cùng có thể nhận được new_package hợp lệ trên route_truck

    assert Function.max_release_date(new_solution[0][index_truck][0][1]) is not None, ""
    assert DIFFERENTIAL_RATE_RELEASE_TIME is not None, ""
    assert Data.standard_deviation is not None, ""

    if (
        Data.release_date[drop_package] == 0
        or Data.city_demand[drop_package] > Data.drone_capacity
    ):
        new_solution[0][index_truck][0][1] += [drop_package]
        stop = True
    elif (
        Function.max_release_date(new_solution[0][index_truck][0][1])
        * DIFFERENTIAL_RATE_RELEASE_TIME
        + Data.standard_deviation
        > Data.release_date[drop_package]
        and Function.min_release_date(new_solution[0][index_truck][0][1])
        * DIFFERENTIAL_RATE_RELEASE_TIME
        + C_ratio * Data.standard_deviation
        > Data.release_date[drop_package]
    ):
        ...
        for m in range(
            len(new_solution[1])
        ):  # Duyệt có điểm nhận hàng nào gộp đợc với new_package không
            for n in range(len(new_solution[1][m])):
                a = new_solution[1][m][n][0]
                if a == max_index:
                    stop1 = True
                ...
                if a in list_check:
                    if (
                        Function.total_demand(new_solution[1][m])
                        + Data.city_demand[drop_package]
                        <= Data.drone_capacity
                        and Function.max_release_date_update(new_solution[1][m])
                        * DIFFERENTIAL_RATE_RELEASE_TIME
                        + Data.standard_deviation
                        ...
                        * DIFFERENTIAL_RATE_RELEASE_TIME
                        ...
                        for c in range(len(new_solution[0][index_truck])):
                            if new_solution[0][index_truck][c][0] == a:
                                ...
                                break
                        new_solution[0][index_truck][
                            number_of_city_change_in_new_truck
                        ][1] += [drop_package]
                        new_solution[1][m][n][1] += [drop_package]
                        break
            if stop1:
    ...
    if not stop:
        ...
        for i in reversed(range(len(new_solution[0][index_truck]))):
            ...
            if index_of_city == drop_package:
                ...
                break
        while (
            Data.euclid_flight_matrix[0][city_received] * 2 + Data.unloading_time
            > Data.drone_limit_time
        ):
            index -= 1
        ...
        if index != 0:
            ...
            new_solution, number_in_the_drone_queue_of_drop_package, index_in_trip = (
                addNewTripInDroneRoute(new_solution, [drop_package], index_truck, index)
            )
    return new_solution
...
def find_shortest_path_by_greedy_1(start, end, list):
    ...
    for i in range(len(list)):
        ...
        for j in range(1, len(new_list)):
            ...
            criteria = (
                Data.manhattan_move_matrix[city_before][city]
                + Data.manhattan_move_matrix[city][city_after]
                - Data.manhattan_move_matrix[city_before][city_after]
            )
...
def find_shortest_path_by_greedy_2(start, end, list):
    ...
    for i in range(len(uncovered_list)):
        ...
        for j in range(len(uncovered_list)):
            ...
            if distance == min_distance:
                if (
                    Data.manhattan_move_matrix[0][current]
                    < Data.manhattan_move_matrix[0][city]
                ):
                    next_city = city
                    min_distance = distance
        current = next_city
        ...
        for j in range(1, len(solution[0][i])):
            for k in range(len(solution[0])):
                for l in range(len(solution[0][k]) - 3):
                    ...
                    if city_change in drop_package:
                        drop_package.remove(city_change)
                    ...
                    new_solution[0][k].insert(
                        begin_index_change_route - 1, [city_change, []]
                    )
                    ...
                    if drop_package != []:  # Nếu điểm bị xóa là điểm nhận hàng
                        ...
                        for mm in range(len(drop_package)):
                            new_solution = findIndexOfDropPackage(
                                new_solution,
                                solution,
                                city_change,
                                truck_time,
                    ...
                    for m in reversed(
                        range(begin_index_change_route, len(new_solution[0][k]))
                    ):
                    ...
                    a, b = find_shortest_path_by_greedy_2(
                        new_solution[0][k][begin_index_change_route - 1][0],
                        0,
                        re_arranged_list,
                    )
                    c, d = find_shortest_path_by_greedy_1(
                        new_solution[0][k][begin_index_change_route - 1][0],
                        0,
                        re_arranged_list,
                    )
                    ...
                    pre_re_arranged_list = find_shortest_path_by_greedy_2(
                        new_solution[0][k][begin_index_change_route - 1][0],
                        0,
                        re_arranged_list,
                    )
                    ...
                    for m in range(len(re_arranged_list)):
                        new_solution[0][k].insert(
                            len(new_solution[0][k]), [re_arranged_list[m], []]
                        )

                    for m in reversed(range(len(next_drop_package))):
                        new_solution = findLocationForDropPackage(
                            new_solution, k, next_drop_package[m]
                        )
                    # print("--------------------------")
                    # print(new_solution[0])
                    # print(new_solution[1])
            ...
            for u in range(len(rs[i][j][1])):
                ...
                if city_change1 != city_change2:
                    ...
                    if newsolution[0][i][pos_2][1] != []:
                        package_needchange = (
                            package_needchange + newsolution[0][i][pos_2][1]
                        )
                    ...
                    for k in range(len(package_needchange)):
                        newsolution[0][i] = Neighborhood11.rearrange_package(
                            package_needchange[k],
                            newsolution[0][i],
                            Function.find_position(
                                package_needchange[k], newsolution[0]
                            )[1],
                        )
...
def Neighborhood_combine_truck_and_drone_neighborhood(
    name_of_truck_neiborhood,
    solution,
    number_of_potiential_solution,
    number_of_loop_drone,
    whether_use_truck_time,
):
    potential_solution = []
    if whether_use_truck_time:
        current_neighborhood = name_of_truck_neiborhood(
            solution, Function.fitness(solution)[1]
        )
    else:
...
def Neighborhood_combine_truck_and_drone_neighborhood_with_tabu_list(
    name_of_truck_neiborhood,
    solution,
    number_of_potial_solution,
    number_of_loop_drone,
    tabu_list,
    tabu_tenure,
    index_of_loop,
    best_fitness,
    kind_of_tabu_structure,
    need_truck_time,
):
    potential_solution = []
    if need_truck_time:
    ...
    for i in range(len(current_neighborhood)):
        ...
        if kind_of_tabu_structure in [1, 2]:
            ...
            if (
                cfsol - best_fitness < epsilon
                or index_of_loop - tabu_list[current_neighborhood[i][2]] > tabu_tenure
            ):
                continue2564 = True
        elif kind_of_tabu_structure in [3, 5]:
            if cfsol - best_fitness < epsilon or (
                index_of_loop - tabu_list[current_neighborhood[i][2][0]] > tabu_tenure
                or index_of_loop - tabu_list[current_neighborhood[i][2][1]]
                > tabu_tenure
            ):
                continue2564 = True
            ...
            if cfsol - best_fitness < epsilon or (
                ...
                or index_of_loop - tabu_list[current_neighborhood[i][2][1]]
                > tabu_tenure
                or index_of_loop - tabu_list[current_neighborhood[i][2][2]]
                > tabu_tenure
            ):
                continue2564 = True
        if continue2564:
...
def Optimize_initial_solution_in_drone(solution):
    ...
    while i < 2:
        i += 1
        neighborhood = Neighborhood_drone.Neighborghood_change_drone_route_max_pro_plus(
            cur_sol
        )
        index = -1
        min_in = 100000
...
def Reverse_truck_route(solution, index_truck):
    ...
    new_solution[0][index_truck] = (
        new_solution[0][index_truck][0:1] + new_solution[0][index_truck][1:][::-1]
    )
...
def sub_tabu_search(solution, loop, index_truck):
    ...
    for i in range(loop):
        neighborhood = []
        if i % 3 < 2:
            neighborhood1 = Neighborhood10.Neighborhood_one_otp_fix_for_specific_truck(
                current_sol, index_truck
            )
            neighborhood2 = (
                Neighborhood10.Neighborhood_move_1_1_ver2_for_specific_truck(
                    current_sol, index_truck
                )
            )
            ...
            neighborhood3 = Neighborhood_drone.Neighborghood_change_drone_route_max_pro_plus_for_specific_truck(
                current_sol, [index_truck]
            )
            ...
            if neighborhood[j][0] == 1:
                for k in range(len(neighborhood[j][1])):
                    ...
                    if cfnode - best_fitness < epsilon:
                        ...
                        and sub_tabu_structure1[neighborhood[j][1][k][2]]
                        + sub_tabu_tenure1
                        <= i
                    ):
                        min_nei[j] = cfnode
                ...
                for k in range(len(neighborhood[j][1])):
                    ...
                    if cfnode - best_fitness < epsilon:
                        ...
                        cfnode - min_nei[j] < epsilon
                        and sub_tabu_structure2[neighborhood[j][1][k][2][0]][
                            neighborhood[j][1][k][2][1]
                        ]
                        + sub_tabu_tenure2
                        <= i
                    ):
        ...
        if neighborhood[index_best_nei][0] == 2:
            ...
            ][neighborhood[index_best_nei][1][index[index_best_nei]][2][1]] = i
            sub_tabu_structure2[
                neighborhood[index_best_nei][1][index[index_best_nei]][2][1]
            ][neighborhood[index_best_nei][1][index[index_best_nei]][2][0]] = i
...
def sub_local_search_optimize(solution):
    ...
    while i - loop_improved > 20:
        neighborhood = one_opt_and_change_truck_route_after(
            solution, current_truck_time
        )
        index = -1
        min_in = 1000000
        for i in range(len(neighborhood)):
...
def Neighborhood_combine_truck_and_drone_neighborhood_with_package(
    name_of_truck_neiborhood,
    solution,
    number_of_potiential_solution,
    number_of_loop_drone,
            ...
            if potential_solution[0][1][0] - min_solution_pack < epsilon:
                ...
                for jjj in range(len(solution_pack)):
                    if jjj + 1 > index_consider_elite_set:
                        if Function.Compare_two_solution_2(
                            potential_solution[0][0], solution_pack[jjj][0]
                        ):
                            have_same_shape = True
            ...
            if potential_solution[0][1][0] - min_solution_pack < epsilon:
                ...
                for jjj in range(len(solution_pack)):
                    if Function.Compare_two_solution_2(
                        ...
                        if jjj + 1 > index_consider_elite_set:
                            if (
                                potential_solution[0][1][0]
                                < solution_pack[jjj][1][0] + epsilon
                            ):
                                add_solution = copy.deepcopy(potential_solution[0])
                                solution_pack[jjj] = add_solution
                                break
                ...
                if not have_same_shape:
                    ...
                    while num1 > index_consider_elite_set:
                        if (
                            potential_solution[0][1][0]
                            < solution_pack[num1 - 1][1][0] + epsilon
                        ):
                            num1 -= 1
                        else:
                            break
...
def Neighborhood_combine_truck_and_drone_neighborhood_with_package_1(
    name_of_truck_neiborhood,
    solution,
    number_of_potiential_solution,
    number_of_loop_drone,
    ...
    for i in range(len(current_neighborhood)):
        ...
        if use_solution_pack:
            if consider:
                ...
                for jjj in range(len(solution_pack)):
                    if Function.Compare_two_solution_2(
                        current_neighborhood[i][0], solution_pack[jjj][0]
                    ):
                        have_same_shape = True
                        if current_neighborhood[i][1][0] < solution_pack[jjj][1][0]:
                            add_solution = copy.deepcopy(current_neighborhood[i])
                ...
                if not have_same_shape:
                    ...
                    while num1 != 0:
                        if (
                            current_neighborhood[i][1][0]
                            < solution_pack[num1 - 1][1][0]
                        ):
                            num1 -= 1
                        else:
                            break
