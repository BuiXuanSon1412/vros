...
def depot_or_after(package, truck_route):
    if (
        Data.manhattan_distance(Data.city[0], Data.city[truck_route[1][0]])
        + Data.unloading_time
        < Data.release_date[package] * 2
    ):
        if (
            Function.sum_weight(truck_route[1][1]) + Data.city_demand[package]
            <= Data.drone_capacity
            and Data.euclid_flight_matrix[0][truck_route[1][0]] * 2
            + Data.unloading_time
...
def over_capacity(truck_route, position):
    ...
    for i in range(len(truck_route[position][1])):
        if (
            Function.sum_weight(truck_route[position][1])
            - Data.city_demand[truck_route[position][1][i]]
            <= Data.drone_capacity
        ):
...
def rearrange_package(package, truck_route, position):
    ...
    if Data.release_date[package] == 0:
        ...
        p = (
            Function.max_release_date(truck_route[nearest_position][1])
            / Data.release_date[package]
        )

        if 1 - alpha <= p <= 1 + alpha:
            if nearest_position != 0:
                if (
                    Function.sum_weight(truck_route[nearest_position][1])
                    + Data.city_demand[package]
                    <= Data.drone_capacity
                ):
                    ...
                    if (
                        Function.sum_weight(
                            truck_route[
                                nearest_resupply(truck_route, nearest_position)
                            ][1]
                        )
                        + Data.city_demand[package_to_go]
                        <= Data.drone_capacity
                    ):
                        truck_route[nearest_resupply(truck_route, nearest_position)][
                            1
                        ].append(package_to_go)
                    else:
                ...
                if (
                    ...
                    + Data.city_demand[package]
                    <= Data.drone_capacity
                    and Data.euclid_flight_matrix[0][truck_route[position][0]] * 2
                    + Data.unloading_time
                    <= Data.drone_limit_time
                ):
            ...
            if maxrd == minrd:
                if truck_route[0][1] != []:
                    ...
                    for i in range(1, position):
                        if (
                            Function.sum_weight(truck_route[i][1])
                            + Data.city_demand[package]
                            <= Data.drone_capacity
                            and Data.euclid_flight_matrix[0][truck_route[i][0]] * 2
                            + Data.unloading_time
                            <= Data.drone_limit_time
                        ):
                ...
                if newposition == 0:
                    ...
                    if (
                        Function.sum_weight(truck_route[newposition][1])
                        + Data.city_demand[package]
                        > Data.drone_capacity
                    ):
                        if (
                            Function.sum_weight(truck_route[newposition - 1][1])
                            + Data.city_demand[package]
                            <= Data.drone_capacity
                            and Data.euclid_flight_matrix[0][
                                truck_route[newposition - 1][0]
                            ]
                            * 2
                            + Data.unloading_time
                            <= Data.drone_limit_time
                        ...
                        if (
                            Data.euclid_flight_matrix[0][truck_route[newposition][0]]
                            * 2
                            + Data.unloading_time
                            <= Data.drone_limit_time
                        ):
...
def fix_drone_queue(newsolution):
        ...
        for k in range(1, len(newsolution[0][h])):
            for l in range(len(newsolution[1])):
                for u in range(len(newsolution[1][l])):
                    if (
                        newsolution[1][l][u][0] == newsolution[0][h][k][0]
                    ):  # Cập nhật các điểm hàng được gộp, hoặc bị xóa
                        if newsolution[0][h][k][1] == []:
                            need_to_earse.append(l)
                        if newsolution[1][l][u][1] == newsolution[0][h][k][1]:
                            continue
                ...
                for u in range(len(newsolution[1])):
                    ...
                    for v in range(len(newsolution[1][u])):
                        if newsolution[0][h][k][0] == newsolution[1][u][v][0]:
                            if newsolution[1][u][v][1] == newsolution[0][h][k][1]:
                                break
                            else:
                                # print(h, k, u)
                                newsolution[1][u][v][1] = copy.deepcopy(
                                    newsolution[0][h][k][1]
                                )
                    ...
                    for e in range(min_pos + 1, max_pos - 1):
                        for q in range(len(newsolution[1][e])):
                            if Function.max_release_date(
                                newsolution[1][e][q][1]
                            ) < Function.max_release_date(newsolution[0][h][k][1]):
                                index = e + 1
        ...
        for u in range(len(newsolution[1][i])):
            for j in range(i + 1, len(newsolution[1])):
                for v in range(len(newsolution[1][j])):
                    if (
                        Function.find_position(newsolution[1][i][u][0], newsolution[0])[
                            0
                        ]
                        == Function.find_position(
                            newsolution[1][j][v][0], newsolution[0]
                        )[0]
                        and Function.find_position(
                            newsolution[1][i][u][0], newsolution[0]
                        )[1]
                        > Function.find_position(
                            newsolution[1][j][v][0], newsolution[0]
                        )[1]
            ...
            for j in range(i + 1, len(newsolution[1])):
                for k in range(len(newsolution[1][j])):
                    if newsolution[1][i][h][0] == newsolution[1][j][k][0]:
                        if (
                            len(newsolution[1][i][h][1]) > Data.drone_capacity
                            or len(newsolution[1][j][k][1]) > Data.drone_capacity
                        ):
    ...
    for i in range(len(removing)):
        newsolution[1][removing[i][0]].remove(
            newsolution[1][removing[i][0]][removing[i][1]]
        )
            ...
            for m in range(len(solution[0])):
                for n in range(1, len(solution[0][m])):
                    if i == m and j == n:
                        ...
                        if sorted([city_change1, city_change2]) in used:
                            continue
                        ...
                        if city_change1 in package_needchange1:
                            package_needchange1.remove(city_change1)
                        if city_change2 in package_needchange1:
                            package_needchange1.remove(city_change2)
                        if city_change2 in package_needchange2:
                            package_needchange2.remove(city_change2)
                        if city_change1 in package_needchange2:
                            package_needchange2.remove(city_change1)

                        # rearrange_package của city_change 1
                        if (
                            city_change1 in solution[0][i][0][1]
                        ):  # Kiểm tra xem giao tại depot hay được resupply
                            if Data.release_date[city_change1] == 0:
                                newsolution[0][m][0][1].append(city_change1)
                            else:
                                p = (
                                    Function.max_release_date(newsolution[0][m][0][1])
                                    / Data.release_date[city_change1]
                                )
                                if p < 1 - beta:
                                    newsolution[0][m] = rearrange_package(
                                        city_change1, newsolution[0][m], n
                                    )
                                else:
                                    newsolution[0][m][0][1].append(city_change1)
                        else:
                        ...
                        for k1 in range(len(package_needchange1)):
                            newsolution[0][i] = rearrange_package(
                                package_needchange1[k1],
                                newsolution[0][i],
                                Function.find_position(
                                    package_needchange1[k1], newsolution[0]
                                )[1],
                            )

                        # rearrange_package của city_change 2
                        if (
                            city_change2 in solution[0][m][0][1]
                        ):  # Kiểm tra xem giao tại depot hay được resupply
                            if Data.release_date[city_change2] == 0:
                                newsolution[0][i][0][1].append(city_change2)
                            else:
                                p = (
                                    Function.max_release_date(newsolution[0][i][0][1])
                                    / Data.release_date[city_change2]
                                )
                                if p < 1 - beta:
                                    newsolution[0][i] = rearrange_package(
                                        city_change2, newsolution[0][i], j
                                    )
                                else:
                                    newsolution[0][i][0][1].append(city_change2)
                        else:
                        ...
                        for k2 in range(len(package_needchange2)):
                            newsolution[0][m] = rearrange_package(
                                package_needchange2[k2],
                                newsolution[0][m],
                                Function.find_position(
                                    package_needchange2[k2], newsolution[0]
                                )[1],
                            )
                        ...
                        a, b, _ = Function.fitness(newsolution)
            ...
            for k in range(len(solution[0])):
                for l in range(1, len(solution[0][k])):
                    ...
                    for ii in reversed(range(len(new_solution[1]))):
                        for iii in reversed(range(len(new_solution[1][ii]))):
                            ...
                            if city == city1 or city == city2:
                                ...
                                for jj in reversed(
                                    range(len(new_solution[1][ii][iii][1]))
                                ):
                    ...
                    for ii in range(len(drop_package1)):
                        new_solution = Neighborhood.findLocationForDropPackage(
                            new_solution, i, drop_package1[ii]
                        )
                    if i != k:
                        for ii in range(len(drop_package2)):
                            new_solution = Neighborhood.findLocationForDropPackage(
                                new_solution, k, drop_package2[ii]
                            )
        ...
        for j in range(len(solution[0][i])):
            for k in range(j + 4, len(solution[0][i])):
                ...
                for ii in range(len(drop_package)):
                    new_solution = Neighborhood.findLocationForDropPackage(
                        new_solution, i, drop_package[ii]
                    )
                    ...
                    if i == k:
                        for ii in range(len(new_solution[0][i])):
                            for iii in reversed(range(len(new_solution[0][i][ii][1]))):
                                ...
                                if (
                                    city == city_change1
                                    or city == city_change2
                                    or city == city_change3
                                ):
                                    new_solution[0][i][ii][1].pop(iii)
                    else:
                        for ii in range(len(new_solution[0][i])):
                    ...
                    for ii in reversed(range(len(new_solution[1]))):
                        for jj in reversed(range(len(new_solution[1][ii]))):
                            ...
                            if (
                                ...
                                for kk in reversed(
                                    range(len(new_solution[1][ii][jj][1]))
                                ):
        ...
        for j in range(1, len(solution[0][i]) - 1):
            for k in range(i + 1, len(solution[0])):
                for l in range(1, len(solution[0][k]) - 1):
                    ...
                    new_solution[0][i] = (
                        new_solution[0][i][: j + 1] + new_solution[0][k][l + 1 :]
                    )
