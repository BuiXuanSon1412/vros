        ...
        for l in range(len(package[i])):
            for j in range(len(package[i][l][1])):
                for k in range(j + 1, len(package[i][l][1])):
                    if (
                        Data.release_date[package[i][l][1][j]]
                        < Data.release_date[package[i][l][1][k]]
                    ):
    ...
    for i in range(len(package)):
        for j in range(len(package[i])):
            for k in range(1, len(package[i][j][1]) + 1):
                ...
                ChangePackages = []
                ChangeInTruck = Function.city_in_which_truck(
                    solution, package[i][j][1][0]
                )
                ...
                for l in range(k):
                    ...
                    # Xoá ở New Solution
                    if package[i][j][0] != 0:
                        NewSolution[1][i - Data.number_of_trucks][j][1].remove(
                            ChangePackage
                        )
                        for ii in range(len(NewSolution[0][ChangeInTruck])):
                            if ChangePackage in NewSolution[0][ChangeInTruck][ii][1]:
                                ...
                                NewSolution[0][ChangeInTruck][ii][1].remove(
                                    ChangePackage
                                )
                                break
                    else:
                ...
                if Function.sum_weight(ChangePackages) > Data.drone_capacity:
                    continue
                if (
                    NewSolution[0][ChangeInTruck][IndexOfReceiveCityInTruck][0]
                    in ChangePackages
                ):
                    continue
                if package[i][j][0] != 0:
                ...
                stop = False
                for l in range(
                    IndexOfReceiveCityInTruck + 1, len(NewSolution[0][ChangeInTruck])
                ):
                    """print("---------------------------------")
                    print(i,", ",j,", ",k,", ",l)"""
                    ...
                    if (
                        Data.euclid_flight_matrix[0][ReceiveCity] * 2
                        + Data.unloading_time
                    ...
                    if New_solution1[0][ChangeInTruck][l][1] == []:
                        ...
                        New_solution1[0][ChangeInTruck][l][1] = (
                            New_solution1[0][ChangeInTruck][l][1] + ChangePackages
                        )
                        ...
                        if (
                            ...
                            Neighborhood.addNewTripInDroneRoute(
                                New_solution1, ChangePackages, ChangeInTruck, l
                            )
                        else:
                            continue
                    # Trường hợp 2: Điểm giao đến cũng là một điểm nhận hàng
                        ...
                        # Add điểm giao hàng tại drone package
                        Neighborhood.groupTripInDroneRoute(
                            New_solution1, ChangePackages, ChangeInTruck, l
                        )
                    """for iii in range(Data.number_of_trucks):
                        print(New_solution1[0][iii])
                    ...
                    for l in range(len(ChangePackages)):
                        ...
                        if package[i][j][0] != 0:
                            ...
                            for ii in range(len(NewSolution[0][ChangeInTruck])):
                                if (
                                    ChangePackage
                                    in NewSolution[0][ChangeInTruck][ii][1]
                                ):
                                    IndexOfReceiveCityInTruck = ii
                                    NewSolution[0][ChangeInTruck][ii][1].remove(
                                        ChangePackage
                    ...
                    for l in range(len(NewSolution[0][ChangeInTruck])):
                        ...
                        if New_solution1[0][ChangeInTruck][l][1] == []:
                            ...
                            for ll in range(len(ChangePackages)):
                                New_solution1[0][ChangeInTruck][l][1] = New_solution1[
                                    0
                                ][ChangeInTruck][l][1] + [ChangePackages[ll]]
                            # Add điểm giao hàng tại drone package
                            if (
                                Data.euclid_flight_matrix[0][ReceiveCity] * 2
                                + Data.unloading_time
                                ...
                                if l != 0:
                                    New_solution1, index_drone_trip, index_in_trip = (
                                        Neighborhood.addNewTripInDroneRoute(
                                            New_solution1,
                                            ChangePackages,
                                            ChangeInTruck,
...
def group_two_trip(
    solution, index_drone_trip1, index_in_trip1, index_drone_trip2, index_in_trip2
):
    ...
    for i in range(len(solution[1])):
        ...
        for j in range(min, max):
            ...
            for k in range(len(solution[1][j])):
                ...
                change_drone_flight_come_truck_number = Function.city_in_which_truck(
                    solution, package_need_group_to_city
                )
                if (
                    Function.total_demand(solution[1][i])
                    + Function.sum_weight(package_need_group)
                    <= Data.drone_capacity
                ):
                    assert Data.standard_deviation is not None, ""
                    if (
                        Function.max_release_date_update(solution[1][i])
                        * DIFFERENTIAL_RATE_RELEASE_TIME
                        + Data.standard_deviation
                        > Function.max_release_date(package_need_group)
                        and Function.min_release_date_update(solution[1][i])
                        * DIFFERENTIAL_RATE_RELEASE_TIME
                        ...
                        for l in range(len(solution[1][i])):
                            ...
                            come_to_truck = Function.city_in_which_truck(
                                solution, new_city_of_package_need_group
                            )
                            if change_drone_flight_come_truck_number == come_to_truck:
                                ...
                                for m in range(len(new_solution[0][come_to_truck])):
                                    ...
                                    if city_evaluate == new_city_of_package_need_group:
                                        new_solution[0][come_to_truck][m][1] += (
                                            package_need_group
                                        )
                                    if city_evaluate == package_need_group_to_city:
                                        for n in range(len(package_need_group)):
                                            new_solution[0][come_to_truck][m][1].remove(
                                                package_need_group[n]
                                            )
                                check = True
                        ...
                        if not check:
                            ...
                            if i < j:
                                for m in range(i + 1, j):
                                    for n in range(len(new_solution[1][m])):
                                        checkCoincideTruck = (
                                            Function.city_in_which_truck(
                                                new_solution, new_solution[1][m][n][0]
                                            )
                                        )
                                        if (
                                            checkCoincideTruck
                                            == change_drone_flight_come_truck_number
                                        ):
                                            stop = True
                                            break
                                    if stop:
                            ...
                            if stop:
                                continue
                            new_solution[1][i].append(
                                [package_need_group_to_city, package_need_group]
                            )
                            shortest_route_by_point, shortest_route_by_truck = (
                                Function.find_drone_flight_shortest(
                                    solution, new_solution[1][i]
                                )
                            )
                            drone_fly_time = (
                                ...
                                + Data.euclid_flight_matrix[
                                    shortest_route_by_point[
                                        len(shortest_route_by_point) - 1
                                    ]
                                ][0]
                            )
                            for m in range(len(shortest_route_by_point) - 1):
                                drone_fly_time += Data.euclid_flight_matrix[
                                    shortest_route_by_point[m]
                                ][shortest_route_by_point[m + 1]]
                            if (
                                drone_fly_time
                                + len(shortest_route_by_point) * Data.unloading_time
                                > Data.drone_limit_time
                            ):
                                continue
        ...
        if not stop1368:
            ...
            for i in range(len(potential_package)):
                ...
                if (
                    ...
                    solution[0][index_truck][index_chosen_city_in_truck][1] += [
                        potential_package[i]
                    ]
                    solution[1][index_drone_trip][index_in_trip][1] += [
                        potential_package[i]
                    ]

        else:
            move_package = solution[1][index_drone_trip_choose_to_group][
                index_in_trip_choose_to_group
            ][1]
            ...
            if not continue771:
                if (
                    demand_package
                    + Function.sum_weight(
                        solution[1][index_drone_trip_choose_to_group][
                            index_in_trip_choose_to_group
                        ][1]
                    )
                    <= Data.drone_capacity
                ):
                    assert Function.max_release_date(stone_package) is not None, ""
                    assert Data.standard_deviation is not None, ""
                    if Function.max_release_date(
                        stone_package
                    ) + Data.standard_deviation > Function.max_release_date(
                        solution[1][index_drone_trip_choose_to_group][
                            index_in_trip_choose_to_group
                        ][1]
                    ):
                        solution = group_two_trip(
                            solution,
                            index_drone_trip,
                            index_in_trip,
                            index_drone_trip_choose_to_group,
                            index_in_trip_choose_to_group,
                        )
    else:
        index_drone_trip_choose_to_group = -1
                ...
                for ii in range(len(Total_case)):
                    ...
                    if package[i][j][0] != 0:
                        if NewSolution[1][i - Data.number_of_trucks][j][1] == []:
                            ...
                            NewSolution = Rearrange_index_trip(
                                NewSolution, i - Data.number_of_trucks, False
                            )
                    stop = False
                    for l in range(len(NewSolution[0][ChangeInTruck])):
                        if l == IndexOfReceiveCityInTruck:
                        ...
                        if New_solution1[0][ChangeInTruck][l][1] == []:
                            ...
                            if (
                                ...
                                if l > IndexOfReceiveCityInTruck:
                                    if (
                                        demand_change_package
                                        <= 0.5 * Data.drone_capacity
                                    ):
                                        New_solution1 = choose_what_to_group(
                                            New_solution1,
                                            index_drone_trip,
                                            index_in_trip,
                                            True,
                                    ...
                                    if (
                                        initial_demand_package - demand_change_package
                                        <= 0.5 * Data.drone_capacity
                                    ):
                                        if (
                                            initial_demand_package
                                            != demand_change_package
                                        ):
                                            if i - Data.number_of_trucks >= 0:
                                                New_solution1 = choose_what_to_group(
                                                    New_solution1,
                                                    i - Data.number_of_trucks,
                                                    j,
                                                    False,
                                                )
                                else:
                        ...
                        # Trường hợp 2: Điểm giao đến cũng là một điểm nhận hàng
                            ...
                            if l != 0:
                                ...
                                if if_group:
                                    NewSolution = Rearrange_index_trip(
                                        NewSolution, index_drone_trip, True
                                    )
                            ...
                            if if_group:  # Gộp thành công
                                if l > IndexOfReceiveCityInTruck:
                                    if (
                                        Function.sum_weight(
                                            New_solution1[1][index_drone_trip][
                                                index_in_trip
                                            ][1]
                                        )
                                        <= 0.5 * Data.drone_capacity
...
def Neighborghood_change_drone_route_max_pro_plus_for_specific_truck(
    solution, accept_truck_list
):
    ...
    for i in range(len(package)):
        for j in range(len(package[i])):
            if package[i][j][0] != 0:
                if (
                    Function.index_truck_of_cities[package[i][j][0]]
                    not in accept_truck_list
                ):
                    continue
            initial_demand_package = Function.sum_weight(package[i][j][1])
            Total_case = takeAmountBeginAndEndPackage(package[i][j][1])
            for ii in range(len(Total_case)):
                ...
                for l in range(len(ChangePackages)):
                    ...
                    # Xoá ở New Solution
                    if package[i][j][0] != 0:
                        NewSolution[1][i - minus_for_number_of_trucks][j][1].remove(
                            ChangePackage
                        )
                        for ii in range(len(NewSolution[0][ChangeInTruck])):
                            if ChangePackage in NewSolution[0][ChangeInTruck][ii][1]:
                ...
                for l in range(len(NewSolution[0][ChangeInTruck])):
                    ...
                    if New_solution1[0][ChangeInTruck][l][1] == []:
                        ...
                        # Add điểm giao hàng tại truck route
                        for ll in range(len(ChangePackages)):
                            New_solution1[0][ChangeInTruck][l][1] = New_solution1[0][
                                ChangeInTruck
                            ][l][1] + [ChangePackages[ll]]
                        # Add điểm giao hàng tại drone package
                        if (
                            Data.euclid_flight_matrix[0][ReceiveCity] * 2
                            <= Data.drone_limit_time
                                ...
                                if initial_demand_package != demand_change_package:
                                    if (
                                        ...
                                        if l != 0:
                                            New_solution1 = choose_what_to_group(
                                                New_solution1,
                                                i - minus_for_number_of_trucks + 1,
                                                j,
                                                True,
                                            )
                                        else:
                    ...
                    # Trường hợp 2: Điểm giao đến cũng là một điểm nhận hàng
                        ...
                        if l != 0:
                            index_drone_trip = -1
                            index_in_trip = -1
                            New_solution1, index_drone_trip, index_in_trip, if_group = (
                                Neighborhood.groupTripInDroneRoute(
                                    New_solution1, ChangePackages, ChangeInTruck, l
                                )
                            )
...
def takeRetrieval(
    Package, ChangePackes, PackageTrip, CurrentNumber, CurrentAmount, TakenAmount
):
    if CurrentAmount == 0:
        ...
        for i in range(
            CurrentNumber, len(PackageTrip) - (TakenAmount - CurrentAmount) + 1
        ):
            Package[CurrentAmount] = PackageTrip[i]
            CurrentNumber = i + 1
            if CurrentAmount == TakenAmount - 1:
...
def Rearrange_index_trip(solution, index_trip, forward):
    ...
    shortest_route_by_point, shortest_route_by_truck = (
        Function.find_drone_flight_shortest(solution, solution[1][index_trip])
    )
    time_fly = (
        Data.euclid_flight_matrix[0][shortest_route_by_point[0]]
    ...
    for i in range(len(solution[1][index_trip])):
        index_truck.append(
            Function.index_truck_of_cities[solution[1][index_trip][i][0]]
        )
    ...
    if forward:
        ...
        while not stop:
            ...
            for i in range(len(solution[1][next_index + 1])):
                if (
                    Function.index_truck_of_cities[solution[1][next_index + 1][i][0]]
                    in index_truck
                ):
                    stop_loop = True
            if stop_loop:
            ...
            if (
                compared_time_fly
                + Function.max_release_date_update(solution[1][next_index])
                < total_time
            ):
                convert = False
            else:
        ...
        if convert:
            ...
            while not stop:
                ...
                for i in range(len(solution[1][next_index - 1])):
                    if (
                        Function.index_truck_of_cities[
                            solution[1][next_index - 1][i][0]
                        ]
                        in index_truck
                    ):
                        stop_loop = True
                if stop_loop:
                    break
                next_index -= 1
                compared_time_fly = Function.cal_time_fly_a_trip(
                    solution[1][next_index]
                )
                if (
                    compared_time_fly
                    + Function.max_release_date_update(solution[1][next_index])
...
def Neighborhood_change_index_trip(solution):
    ...
    for i in range(len(solution[1])):
        ...
        for j in range(i + 1, len(solution[1])):
            ...
            for k in range(len(solution[1][j])):
                if (
                    Function.index_truck_of_cities[solution[1][j][k][0]]
                    in list_city_stop
                ):
                    stop = True
            if stop:
