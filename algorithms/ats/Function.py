...
def initial_solution():
    ...
    for i in range(Data.number_of_trucks + 1, Data.number_of_cities):
        ...
        for j in range(Data.number_of_trucks):
            if (
                Data.manhattan_move_matrix[route[j][len(route[j]) - 1]][
                    next_customer_add
                ]
                < min
                and i - restrict[j] > Data.number_of_trucks * 0.5
            ):
                min = Data.manhattan_move_matrix[route[j][len(route[j]) - 1]][
                    next_customer_add
                ]
                index = j
        restrict[index] = i
...
def initial_solution1():
    ...
    r_d.pop(0)
    r_d = sorted(r_d, key=lambda r_d: r_d[1])
    truck_time = [0.0 for _ in range(Data.number_of_trucks)]
    for i in range(Data.number_of_trucks):
        route[i].append(r_d[0][0])
        truck_time[i] += (
            Data.release_date[r_d[0][0]] + Data.manhattan_move_matrix[0][r_d[0][0]]
        )
        #       crd.append(r_d[i][0])
        r_d.pop(0)
    for i in range(Data.number_of_trucks + 1, Data.number_of_cities):
        ...
        for j in range(Data.number_of_trucks):
            if (
                abs(
                    Data.euclid_flight_matrix[route[j][len(route[j]) - 1]][
                        next_customer_add
                    ]
                    + r_d[0][1]
                    - truck_time[j]
                ...
                index = j
        restrict[index] = i
        truck_time[index] = max(
            float(
                Data.release_date[r_d[0][0]] + Data.euclid_flight_matrix[0][r_d[0][0]]
            ),
            float(
                truck_time[index]
                + Data.manhattan_move_matrix[route[index][len(route[index]) - 1]][
                    next_customer_add
                ]
            ),
        )
...
def initial_nearest_neighbor_resupply():
    ...
    for _ in range(Data.number_of_cities - 1):
        ...
        for city in range(Data.number_of_cities):
            if (
                not visited[city]
                and Data.manhattan_move_matrix[current_city][city] < min_distance
            ):
                nearest_city = city
    ...
    for i in range(Data.number_of_trucks):
        ...
        for ii in range(len(list_for_truck[i])):
            list_1.append(
                [
                    list_for_truck[i][ii],
                    Data.manhattan_move_matrix[0][list_for_truck[i][ii]],
                ]
            )
...
def initial_solution5():
    ...
    for i in range(60):
        ...
        a = random.random()
        if a < 0.5:
            neighborhood1 = Neighborhood_for_TSP.Neighborhood_move_1_0_no_drone(
                current_sol
            )
            neighborhood2 = Neighborhood_for_TSP.Neighborhood_move_1_1_no_drone(
                current_sol
            )
            neighborhood.append([1, neighborhood1])
            neighborhood.append([2, neighborhood2])
        else:
            neighborhood3 = Neighborhood_for_TSP.Neighborhood_move_2_0_no_drone(
                current_sol
            )
            neighborhood4 = Neighborhood_for_TSP.Neighborhood_move_2_1_no_drone(
                current_sol
            )
            ...
            if neighborhood[j][0] == 1:
                for k in range(len(neighborhood[j][1])):
                    ...
                    if cfnode - best_fitness < epsilon:
                        ...
                        and tabu_structure1[neighborhood[j][1][k][2]] + tabu_tenure1
                        <= i
                    ):
                        min_nei[j] = cfnode
                        index[j] = k
                ...
                for k in range(len(neighborhood[j][1])):
                    ...
                    elif cfnode - min_nei[j] < epsilon and (
                        tabu_structure2[neighborhood[j][1][k][2][0]] + tabu_tenure2 <= i
                        or tabu_structure2[neighborhood[j][1][k][2][1]] + tabu_tenure2
                        <= i
                    ):
                        min_nei[j] = cfnode
                        index[j] = k
                ...
                for k in range(len(neighborhood[j][1])):
                    ...
                    if cfnode - best_fitness < epsilon:
                        ...
                        cfnode - min_nei[j] < epsilon
                        and tabu_structure3[neighborhood[j][1][k][2][1]] + tabu_tenure3
                        <= i
                        and tabu_structure3[neighborhood[j][1][k][2][0]] + tabu_tenure3
                        <= i
                    ):
                        min_nei[j] = cfnode
                        index[j] = k
                ...
                for k in range(len(neighborhood[j][1])):
                    ...
                    if cfnode - best_fitness < epsilon:
                        ...
                        tabu_structure4[neighborhood[j][1][k][2][1]] + tabu_tenure4 <= i
                        or tabu_structure4[neighborhood[j][1][k][2][0]] + tabu_tenure4
                        <= i
                        or tabu_structure4[neighborhood[j][1][k][2][2]] + tabu_tenure4
                        <= i
                    ):
                        min_nei[j] = cfnode
                        index[j] = k
        ...
        if neighborhood[index_best_nei][0] == 1:
            tabu_structure1[
                neighborhood[index_best_nei][1][index[index_best_nei]][2]
            ] = i

        if neighborhood[index_best_nei][0] == 2:
            tabu_structure2[
                neighborhood[index_best_nei][1][index[index_best_nei]][2][1]
            ] = i
            tabu_structure2[
                neighborhood[index_best_nei][1][index[index_best_nei]][2][0]
            ] = i
        ...
        if neighborhood[index_best_nei][0] == 4:
            ...
            tabu_structure4[
                neighborhood[index_best_nei][1][index[index_best_nei]][2][2]
            ] = i

    for i in range(len(best_sol[0])):
        for j in range(1, len(best_sol[0][i])):
            best_sol = Neighborhood.findLocationForDropPackage(
                best_sol, i, best_sol[0][i][j][0]
            )
            # print(best_sol)
            # print("------------", j, "------------")
    ...
    for i in range(30):
        ...
        neighborhood1 = (
            Neighborhood_drone.Neighborghood_change_drone_route_max_pro_plus(
                current_sol
            )
        )
...
def initial_solution6():
    ...
    for i in range(len(init_solution[0])):
        for j in range(1, len(init_solution[0][i])):
            init_solution = Neighborhood.findLocationForDropPackage(
                init_solution, i, init_solution[0][i][j][0]
            )
...
def initial_solution7():
    ...
    tabu_tenure4 = tabu_tenure1 = tabu_tenure3 = tabu_tenure2 = random.uniform(
...
def find_drone_flight_route(truck_position):
    ...
    while len(truck_position) != 0:
        ...
        for i in range(len(truck_position)):
            if (
                Data.euclid_flight_matrix[current_point][truck_position[i]]
                <= current_len
            ):
                current_len = Data.euclid_flight_matrix[current_point][
                    truck_position[i]
                ]
...
def cal_time_fly_a_trip(package):
    ...
    time_fly = (
        Data.euclid_flight_matrix[0][shortest_route_by_point[0]]
        + Data.euclid_flight_matrix[shortest_route_by_point[-1]][0]
    )
    for i in range(len(shortest_route_by_point) - 1):
        time_fly += Data.euclid_flight_matrix[shortest_route_by_point[i]][
            shortest_route_by_point[i + 1]
        ]
...
def init_queue(solution):
    ...
    print(f"No. drones: {Data.number_of_drones}")
    ...
    for i in range(Data.number_of_trucks):
        ...
        truck_time[i] = float(
            max_release_date(solution[i][truck_position[i][truck_current_point[i]]][1])
            + Data.manhattan_move_matrix[truck_position[i][truck_current_point[i]]][
    ...
    while True:
            ...
            if truck_position[i][truck_current_point[i]] == 0:
                ...
                for j in range(len(fake)):
                    ...
                    if size <= Data.drone_capacity:
                        temp.append(fake[j])
                compare[i] = max(
                    max_release_date(temp),
                    truck_time[i]
                    - Data.euclid_flight_matrix[
                        0, truck_position[i][truck_current_point[i]]
                    ],
                )
        number = 0
        for i in range(Data.number_of_trucks):
        ...
        drone_queue.put(
            (
                ...
                + 2
                * Data.euclid_flight_matrix[
                    truck_position[index][truck_current_point[index]]
                ][0],
                drone[1],
            )
        )
        ...
        while solution[index][truck_current_point[index]][1] == []:
            if truck_position[index][truck_current_point[index]] == 0:
                break
            ...
            if num == 0:
                truck_time[index] = (
                    max(truck_time[index], start)
                    + Data.euclid_flight_matrix[
                        truck_position[index][truck_current_point[index]]
                    ][0]
                    + Data.manhattan_move_matrix[
                        truck_position[index][truck_current_point[index]]
                    ][truck_position[index][truck_current_point[index] + 1]]
                )
            else:
                truck_time[index] = (
                    truck_time[index]
...
def cal_distance_of_truck(solution, index_truck):
    ...
    for i in range(index_of_last_city_in_truck):
        total_distance += Data.manhattan_move_matrix[solution[0][index_truck][i][0]][
            solution[0][index_truck][i + 1][0]
        ]
    total_distance += Data.manhattan_move_matrix[
        solution[0][index_truck][index_of_last_city_in_truck][0]
    ][solution[0][index_truck][0][0]]
...
def fitness(solution):
    ...
    # Declare
    truck_time = [0.0] * Data.number_of_trucks
    truck_position = []
    temp = []
    ...
    for i in range(Data.number_of_trucks):
        ...
        if len(truck_position[i]) != 2:
            ...
            truck_time[i] = float(
                max_release_date(
                    base_path[i][truck_position[i][truck_current_point[i]]][1]
                )
                + distance
            )
    ...
    while True:
        for i in range(Data.number_of_trucks):
            while base_path[i][truck_current_point[i]][1] == []:
                ...
                """print("Truck", i, "move from", truck_position[i][truck_current_point[i]], "to", truck_position[i][truck_current_point[i] + 1])"""
                truck_time[i] = float(truck_time[i] + distance)
                data_truck[i].append(truck_time[i] - distance)
        ...
        start = max(
            drone[0], max_release_date(drone_pack)
        )  # Thời gian drone có thể xuất phát
        LastCityOfDrone = -1
        for i in range(len(position)):
            ...
            if i == 0:
                ...
                LastCityOfDrone = truck_position[position[i]][
                    truck_current_point[position[i]]
                ]
            else:
                ...
                start = (
                    start
                    + Data.euclid_flight_matrix[
                        truck_position[position[i - 1]][
                            truck_current_point[position[i - 1]]
                        ]
                    ][truck_position[position[i]][truck_current_point[position[i]]]]
                )
                LastCityOfDrone = truck_position[position[i]][
            ...
            while base_path[position[i]][truck_current_point[position[i]]][1] == []:
                ...
                if num == 0:
                    ...
                    truck_time[position[i]] = truck_time[position[i]] + distance
                    data_truck[position[i]].append(truck_time[position[i]] - distance)
                num = num + 1
                if (
                    truck_position[position[i]][truck_current_point[position[i]] + 1]
...
def Check_if_feasible(solution):
    ...
    #            if truck_check1[truck_index].index(solution[1][i][j][0]) > truck_check1[
    #                truck_index
    #            ].index(solution[1][i][j][1][k]):
    #                print("Nhận hàng sau vị trí khách hàng")
    #                return False
    # Check if the pickup location is after the customer's delivery location

    for i in range(len(solution[1])):
        trips = solution[1][i]  # Intermediate variable for the trip
        for j in range(len(trips)):
            pickup_node = trips[j][0]
            delivery_packages = trips[j][1]

            truck_index = city_in_which_truck(solution, pickup_node)

            # Ensure truck_index is valid before proceeding
            if truck_index is None:
                continue

            for k in range(len(delivery_packages)):
                # Extract individual package ID
                package_id = delivery_packages[k]

                # Now the indices are clear to the type checker
                pickup_idx = truck_check1[truck_index].index(pickup_node)
                delivery_idx = truck_check1[truck_index].index(package_id)
        ...
        for j in range(i + 1, len(drone_queue)):
            if drone_queue[i] == drone_queue[j]:
                ...
                for k in range(i + 1, j):
                    if (
                        truck_index == city_in_which_truck(solution, drone_queue[k])
                        and drone_queue[k] != drone_queue[j]
                    ):
                        print("Thứ tự giao hàng sai")
                        print(drone_queue[i], "----", drone_queue[j])
                        return False
                if city_coincide.count(drone_queue[j]) == 0:
                    if j not in city_coincide:
            ...
            while k < len(shortcut_solution1[i]):
                ...
                for p in range(start_sh_sol2, min(len(shortcut_solution2[j]), k + 4)):
                    if shortcut_solution1[i][k] == shortcut_solution2[j][p]:
                        ...
                        break
                if go:
                    while pp + 1 < len(shortcut_solution2[j]) and k + k_plus < len(
                        shortcut_solution1[i]
                    ):
                        if (
                            shortcut_solution2[j][pp + 1]
                            == shortcut_solution1[i][k + k_plus]
                        ):
                            k_plus += 1
                            pp += 1
                            if k_plus > 2:
...
def schedule(solution):
    """
    Calculates arrival and departure times for each node in the solution.
    Returns the solution in a dictionary-based format.
    """
    # 1. Initialize data structures
    drone_package = copy.deepcopy(solution[1])
    base_path = copy.deepcopy(solution[0])

    # Track current state for each truck
    truck_time = [0.0] * Data.number_of_trucks
    truck_current_point = [0] * Data.number_of_trucks

    # Expanded schedules
    # truck_schedules[i] = list of stops for truck i
    truck_schedules = [[] for _ in range(Data.number_of_trucks)]
    # drone_schedules = list of trip details
    drone_schedules = []

    # Map the full path for each truck (including final return to depot)
    truck_position = []
    for i in range(len(base_path)):
        nodes = [stop[0] for stop in base_path[i]]
        nodes.append(0)  # Return to depot
        truck_position.append(nodes)

    # Initialize drones
    drone_queue = queue.PriorityQueue()
    for i in range(Data.number_of_drones):
        drone_queue.put((0.0, f"Drone {i}"))

    # 2. Initial movement from Depot
    for i in range(Data.number_of_trucks):
        if len(truck_position[i]) > 2:
            curr_node = 0
            # Get packages loaded at depot (from index 0 of base_path)
            pkgs = copy.deepcopy(base_path[i][0][1])

            arrival = 0.0
            departure = float(max_release_date(pkgs))

            truck_schedules[i].append(
                {
                    "node": curr_node,
                    "arrive": arrival,
                    "depart": departure,
            ...
            # Move to next node
            next_node = truck_position[i][1]
            dist = Data.manhattan_move_matrix[curr_node][next_node]
            truck_time[i] = departure + dist

            # Clear packages at depot node in our working copy
            base_path[i][0][1] = []
            truck_current_point[i] = 1
        else:
            # Handle empty routes
            truck_schedules[i].append(
                {"node": 0, "arrive": 0.0, "depart": 0.0, "packages": []}
            )

    # 3. Main Simulation Loop (based on fitness logic)
    while True:
        ...
        for i in range(Data.number_of_trucks):
            while base_path[i][truck_current_point[i]][1] == []:
                ...
                arrival = truck_time[i]
                departure = arrival + Data.unloading_time

                # Find original packages for this stop if any were truck-delivered
                original_pkgs = (
                    solution[0][i][truck_current_point[i]][1]
                    if truck_current_point[i] < len(solution[0][i])
                    else []
                )
                ...
                next_idx = truck_current_point[i] + 1
                dist = Data.manhattan_move_matrix[curr_node][
                    truck_position[i][next_idx]
                ]
                truck_time[i] = float(departure + dist)
        ...
        if all(
            truck_position[i][truck_current_point[i]] == 0
            for i in range(Data.number_of_trucks)
        ):
            break

        # Step C: Handle Drone Trips
        if drone_package:
            current_trip = drone_package.pop(0)
            all_trip_pkgs = [p for stop in current_trip for p in stop[1]]

            # Determine flight path and which trucks are involved
            flight_path, truck_indices = find_drone_flight_shortest(
                solution, current_trip
            )

            # Get available drone
            drone_avail, drone_id = drone_queue.get()
            # Drone leaves depot only when packages are released
            drone_start_time = max(drone_avail, max_release_date(all_trip_pkgs))

            drone_curr_time = drone_start_time
            last_drone_node = 0
            trip_details = []

            for idx, truck_idx in enumerate(truck_indices):
                target_node = truck_position[truck_idx][truck_current_point[truck_idx]]

                # Flight leg
                drone_curr_time += Data.euclid_flight_matrix[last_drone_node][
                    target_node
                ]

                # Truck state at meeting point
                ...
                # Identify packages delivered in this specific stop
                deliver = [
                    p
                    for stop in current_trip
                    if stop[0] == target_node
                    for p in stop[1]
                ]

                # Record Truck Schedule Update
                truck_schedules[truck_idx].append(
                    {
                        "node": target_node,
                        "arrive": truck_arrival,
                        "depart": finish_time,
                        "packages": deliver,
                    }
                )

                # Mark packages as delivered in working path
                for p in deliver:
                    for stop in base_path[truck_idx]:
                        if p in stop[1]:
                            stop[1].remove(p)
            ...
            drone_end_time = (
                drone_curr_time + Data.euclid_flight_matrix[last_drone_node][0]
            )
            drone_schedules.append(
                {
                    "drone_id": drone_id,
            ...
            drone_queue.put((drone_end_time, drone_id))

    return [truck_schedules, drone_schedules]
