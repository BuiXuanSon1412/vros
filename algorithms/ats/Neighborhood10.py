            ...
            for k in range(len(solution[0])):
                for l in range(len(solution[0][k])):
                    ...
                    if i == k and (j == l or j == l + 1):
                        ...
                        if city_change in package_need_change:
                            package_need_change.remove(city_change)
                        ...
                        if (
                            package_need_change != []
                        ):  # Nếu điểm bị xóa là điểm nhận hàng
                            erase_list = []
                                ...
                                for n in range(len(new_solution[1][m])):
                                    if new_solution[1][m][n][0] == city_change:
                                        ...
                                        for p in range(len(new_solution[1][m][n][1])):
                                            ...
                                            package_need_change1.append(
                                                new_solution[1][m][n][1][p]
                                            )
                            ...
                            for mm in range(len(package_need_change1)):
                                new_solution = Neighborhood.findIndexOfDropPackage(
                                    new_solution,
                                    solution,
                                    city_change,
                                    truck_time,
                        ...
                        assert (
                            Function.max_release_date(new_solution[0][k][0][1])
                            is not None
                        ), ""
                        ...
                        if Data.release_date[city_change] == 0:
                            ...
                            * DIFFERENTIAL_RATE_RELEASE_TIME
                            ...
                            > Data.release_date[city_change]
                            and Function.min_release_date(new_solution[0][k][0][1])
                            * DIFFERENTIAL_RATE_RELEASE_TIME
                                ...
                                for n in range(len(new_solution[1][m])):
                                    ...
                                    if a in list_check:
                                        if (
                                            ...
                                            + Data.city_demand[city_change]
                                            <= Data.drone_capacity
                                            and Function.max_release_date_update(
                                                new_solution[1][m]
                                            )
                                            * DIFFERENTIAL_RATE_RELEASE_TIME
                                            + Data.standard_deviation
                                            > Data.release_date[city_change]
                                            and Function.min_release_date_update(
                                                new_solution[1][m]
                                            )
                                            * DIFFERENTIAL_RATE_RELEASE_TIME
                        ...
                        if not stop:  # Nếu việc gộp vào các gói hàng trước không được, tạo điểm nhận hàng mới cho new_package
                            add1 = Data.manhattan_move_matrix[solution[0][k][l][0]][
                                city_change
                            ]
                            add2 = 0
                                ...
                                if a > l + 1:
                                    ...
                                    if (
                                        ...
                                        if criterion < minn:
                                            index = a
                                            minn = criterion
                                if new_solution[0][k][a][0] == city_change:
                                    break
                            ...
                            if index != 0:
                                (
                                    new_solution,
                                    number_in_the_drone_queue_of_drop_package,
                                    index_in_trip,
                                ) = Neighborhood.addNewTripInDroneRoute(
                                    new_solution, [city_change], k, index
                                )
            ...
            for k in range(len(solution[0])):
                for l in range(len(solution[0][k])):
                    ...
                    if i == k and (j == l or j == l + 1):
                        ...
                        # Xóa city change trong drop_package (drop_package là các gói hàng nhận tại city_change)
                        if city_change in pre_drop_package:
                            pre_drop_package.remove(city_change)
                        # Xóa city_change ra khỏi truck route
                        new_solution[0][i].pop(j)
                        ...
                        for m in reversed(range(len(new_solution[1]))):
                            for mm in reversed(range(len(new_solution[1][m]))):
                                for mmm in reversed(
                                    range(len(new_solution[1][m][mm][1]))
                                ):
                        ...
                        for m in range(len(drop_package)):
                            new_solution = Neighborhood.findLocationForDropPackage(
                                new_solution, i, drop_package[m]
                            )

                        # Xử lý gói hàng của city change được giao ở đâu tại trip drone nào ?
                        new_solution = Neighborhood.findLocationForDropPackage(
                            new_solution, k, city_change
                        )
                        # print(new_solution)
                        # print(Function.Check_if_feasible(new_solution))
                        pack_child = []
                ...
                for l in range(len(solution[0][k])):
                    ...
                    if i == k and (j == l or j == l + 1):
                        ...
                        if drop_package != []:  # Nếu điểm bị xóa là điểm nhận hàng
                            ...
                            while (
                                Data.euclid_flight_matrix[0][city_received] * 2
                                + Data.unloading_time
                                > Data.drone_limit_time
                            ):
                                IndexOfNewReceiveCity -= 1
                                city_received = new_solution[0][i][
                                    IndexOfNewReceiveCity
                                ][0]

                            for mm in range(len(drop_package1)):
                                new_solution[0][i][IndexOfNewReceiveCity][1] = (
                                    new_solution[0][i][IndexOfNewReceiveCity][1]
                                    + drop_package1[mm]
                                )
